...
function initializeMultiStepForm() {
  ...
  if (gradeSelect) {
    gradeSelect.addEventListener("change", () => {
      // Reload classes for the selected grade.
      // Admission number is NOT re-fetched here — grade is no longer part of the number.
      _loadClassesForGrade(gradeSelect.value);
      // Clear the admission number field so the user knows to select a class next
      const admInput = document.getElementById("admissionNumber");
      if (admInput) { admInput.value = ""; admInput.placeholder = "Auto-assigned after selecting a Class"; }
    });
...
// The admission number is always generated by the backend.
// Format: {PREFIX}-{YEAR}-{NNNN}  e.g. ADM-2026-0001 or 9736-2026-0001
// PREFIX comes from school_settings.school_code (falls back to ADM).
// Class, grade and stream are NOT part of the number.
let _admFetchSeq = 0;
...
async function _updateAutoAdmissionNumber() {
  ...
  if (!classId) {
    admInput.value = "";
    admInput.placeholder = "Auto-assigned after selecting a Class";
    return;
  }

  admInput.value = "";
  admInput.placeholder = `Fetching admission number for ${year}…`;

  const currentSeq = ++_admFetchSeq;
  try {
    const res = await apiFetch(`/api/students/next-admission-number?year=${year}`);
    ...
    // API responded but returned no number
    admInput.placeholder = "Could not fetch admission number — please retry";
  } catch (err) {
    console.warn("_updateAutoAdmissionNumber API error:", err);
    if (currentSeq === _admFetchSeq) {
      admInput.placeholder = "Network error — admission number will be assigned on save";
    }
  }
}
